...
**Income, Rent, Population, and Households ACS Tables:** The temporal variable year and the geographic identifier GEOID are shared by these four American Community Survey datasets. We can align data like median household income, average monthly rent, total population, and number of households for each metropolitan region and year thanks to these shared keys. Direct joins are simple because all four databases have the same structure and composite primary keys (GEOID, year). Due to COVID-19 survey disruptions, 2020 is not included in the data, which covers the years 2009–2023.

**PERMITS (Building Permits Survey):** Identifies metropolitan areas using the CBSA code. Although GEOID is a string and CBSA is an integer, they represent similar regional borders and can be cross-referenced to link new dwelling development data with the ACS demographic indicators. R's automatic type conversion in join operations is used to convert CBSA to match GEOID format, which is necessary for the join. The number of new housing units approved annually in each metro region is shown in this dataset.

**WAGES (Bureau of Labor Statistics QCEW):** Contains variables for total employment, total wages, and average yearly wage by industry (INDUSTRY) and uses the FIPS code to identify regions. Because the FIPS codes include a "C" prefix (e.g., "C12345" instead than "12345"), string alteration is necessary before merging with Census data. Furthermore, this dataset has uppercase column naming rules (YEAR rather than year), which calls for caution while joining. A long-format structure is produced by the many appearances of each metro-year observation in WAGES, one for each industry sector. The lookup table INDUSTRY_CODES has comprehensive descriptions linked to each INDUSTRY code, enabling examination of certain occupational areas such as technology, healthcare, or finance.

Using the North American Industry Classification System (NAICS) to convert the numerical codes in the WAGES dataset to descriptive names, INDUSTRY_CODES (Lookup Table) offers hierarchical industry classification labels. There are four levels in the hierarchy, ranging from general economic sectors (level 1) to specialized industries (level 4). Level 1 might be "Professional Services," for instance, and level 4 might be "Data Science and Business Analytics." This lookup table makes it possible to interpret employment and salary trends across many occupational groups in a way that is legible by humans.

When combined, these links demonstrate how careful data modification can merge various data sources with different identifiers (GEOID, CBSA, FIPS) and varied formatting rules. Type conversion (string vs. integer), string manipulation (prefix removal), and column name alignment (case sensitivity) are the main obstacles. We have enough linking keys to examine how income, rent, housing supply, and wage dynamics interact across time in U.S. metropolitan areas despite these technical obstacles. This integrated dataset makes it possible to analyze how employment growth in various industries relates to housing demand, whether metro areas that allow more housing construction have lower rent burdens, and which metropolitan areas have successfully balanced population growth with housing supply expansion.
